...
// Error codes and user-friendly messages (i18n-aware)
// Returns translated messages when i18n is available, falls back to English
function getErrorMessages() {
  const _t = window.SmartPRI18n ? window.SmartPRI18n.t : () => null;
  return {
    USER_NOT_AUTHORIZED: _t('apiError.notAuthorized') || 'Please enter your Smart.pr email address in the extension settings.',
    INVALID_EMAIL: _t('apiError.invalidEmail') || 'Please enter a valid email address in the extension settings.',
    RATE_LIMIT_EXCEEDED: _t('apiError.rateLimit') || "You've reached your usage limit. Please try again later.",
    OPENAI_ERROR: _t('apiError.openai') || 'Our AI service encountered an error. Please try again.',
    SERVER_ERROR: _t('apiError.server') || 'Our service is temporarily unavailable. Please try again in a moment.',
    NETWORK_ERROR: _t('apiError.network') || 'Connection failed. Please check your internet connection.',
    EXTENSION_CONTEXT_INVALIDATED: _t('apiError.extensionUpdated') || 'Extension updated. Refresh the page to continue.',
    INVALID_REQUEST: _t('apiError.invalidRequest') || 'Invalid request. Please try again.',
    UNKNOWN_ERROR: _t('apiError.unknown') || 'An unexpected error occurred. Please try again.'
  };
}

// Backward-compatible: ERROR_MESSAGES as a getter proxy
